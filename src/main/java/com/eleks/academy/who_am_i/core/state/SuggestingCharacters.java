...
public class SuggestingCharacters extends AbstractGameState {
    ...
    public GameState makeTurn(Answer answer) throws GameException {
        ...
        try {
            return Optional.of(answer)
                    ...
                    .map(then -> this.next()))
                    .orElseGet(() -> this.suggestCharacter(answer.getPlayer(),
                    answer.getMessage()));
        } finally {
            this.lock.unlock();
        }
    ...
    private GameState assignCharacters() {
        ...
        authors.forEach(author -> {
            final var character = this.getRandomCharacter()
                    .apply(this.suggestedCharacters.get(this.
                            <String>cyclilNext().apply(authors, author)));
        ...
        this.players.keySet()
                ...
                .filter(player -> {
                    final var charater =
                            this.getRandomCharacter().apply(nonTakenCharacters);
                    character.markTaken();
